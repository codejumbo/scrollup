...
Scaffold a new site with `create-scrollup`, install dependencies, and start the dev server.
...
<Tabs labels={["npm", "pnpm", "yarn", "bun"]}>
  ...
  <Tab index={1}>
    ```sh title="Terminal"
    pnpm create scrollup@latest my-docs
    cd my-docs
    pnpm install
    pnpm run dev
    ```
...
<Steps>
  <Step num={1} title="Scaffold and install">
    Scaffold the template and install dependencies.
